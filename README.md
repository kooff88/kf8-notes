# TODO
  笔记    这里放置我的笔记，搜索，实践，摘录

## javascript
- [基础](javascript/基础.md)
- [回调函数](javascript/回调函数.md)
- [常用正则](javascript/常用正则.md)
- [es6](javascript/es6.md)
- [FileReader接口](javascript/FileReader接口.md)

## 算法
- [动态规则](算法/动态规则.md)
- [B + 树](算法/B + 树.md)
- [B - 树](算法/B - 树.md)

## CSS3 
- [基础](css3/基础.md)
- [时间4种状态](css3/时间4种状态.md)

## GIT 
- [git各种用法](git/git.md)
 
## MySQL
- [mysql](mysql/mysql.md)
- [搜索优化－mysql](#mysql/search_mysql.md)
  
## NODE
- [node笔记](node/node.md)
- [支付宝](node/支付宝.md)
- [权限](node/权限.md)

## linux
- [mac常用命令](linux/mac常用命令.md)


## REACT.JS
- [react.js](reactjs/reactjs.md)
- [弹出层封装](reactjs/弹出层封装.md)
- [生命周期](reactjs/生命周期.md)

## ReactNative
 - [运用react.native笔记](reactNative/reactNative.md)
 - [分享功能](reactNative/分享.md)
  
## redux
 - [bindActionCreators](redux,react-redux/bindActionCreators.md)
 - [react-redux中connect](redux,react-redux/react-redux中connect.md)
## other
 - [运用postman笔记](other/postman.md)
 - [运用redis笔记](other/redis.md)
 - [运用siege网站测试工具笔记](other/siege.md)
 - [shell-zsh加载](other/shell-zsh.md)
 - [sublime插件安装 && subl ./](other/sublime.md)
 - [shell语法](other/shell脚本语法.md)